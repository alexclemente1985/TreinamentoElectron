...
export function Sidebar(){
    ...
    return(
        ...
        className="bg-gray-950 flex-shrink-0 border-r border-slate-600 h-screen relative group overflow-hidden data-[state=open]:animate-slideIn data-[state=closed]:animate-slideOut">
            <Collapsible.Trigger
            className={
                clsx('absolute h-7 w-7 text-white hover:scale-105 right-4 z-[99] text-slate-200 inline-flex items-center justify-center',
                    {
                        'top-[1.124rem]':isMacOS,
                        'top-6': !isMacOS
                    }
                )
            }>
            ...
            <div
            className={clsx('region-drag z-0 mt-10',{
                block: isMacOS,
                hidden: !isMacOS
            })}></div>
